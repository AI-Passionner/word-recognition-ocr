...
class ImageUtils:
    ...
    @staticmethod
    def reverse_bbox(img_h, img_w, bbox):
        x0, y0, x1, y1 = bbox
        x1 = int(x1 * img_w)
        y1 = int(y1 * img_h)
        x0 = int(x0 * img_w)
    ...
    @staticmethod
    def remove_lines(img_gray, hori_size=120, vert_size=100):  # in selected ROI, which might be easy
        """
        The larger hori_size/vert_size, the less lines removed. However, the smaller hori_size/vert_size
        ...
        hori_structure = cv2.getStructuringElement(cv2.MORPH_RECT, (hori_size, 1))
        horizontal = cv2.erode(horizontal, hori_structure, (-1, -1))
        horizontal = cv2.dilate(horizontal, hori_structure, (-1, -1))
        ...
        vertical = cv2.erode(vertical, vert_structure, (-1, -1))
        vertical = cv2.dilate(vertical, vert_structure, (-1, -1))
